...
use lightyear::netcode::ServerConfig;
...
use crate::networking::protocol::{make_reusable_udp_socket, DISCOVERY_ADDR, DISCOVERY_CLIENT_MAGIC, DISCOVERY_PORT, UNSPECIFIED};
            ...
            Ok((len, SocketAddr::V4(addr))) => {
                if &buf[..len] == DISCOVERY_CLIENT_MAGIC {
                    ...
                    if let Err(e) = socket.socket.send_to(
                        format!("OKBRO\nNAME {}\nIP {}:{}\n\n\n",server.0.0, server_addr.ip(), server.1.0.port()).as_bytes()
                        ,addr) {
...
pub fn start_server(
    ...
    if let Ok(socket) = make_reusable_udp_socket(DISCOVERY_PORT) {
        ...
        let mut local_addr: LocalAddr = LocalAddr(SocketAddrV4::new(Ipv4Addr::UNSPECIFIED, 0).into());
        {
